...
public class HoaDonCTDAO {
  public void insert(ChiTietHD model) {
    ...
    public void update(ChiTietHD model){
        String sql="UPDATE ChiTietHoaDon SET MaSP=?, TenSP=?, DonGia=?, SoLuong=?, TongTien=?, GhiChu=? WHERE MaHD=?";
    JdbcHelper.executeUpdate (sql , model.getMaSP(),model.getTenSP(),model.getDonGia(),model.getSoLuong(),model.getTongTien(),model.getGhiChu(),model.getMaHD()); 
  }
